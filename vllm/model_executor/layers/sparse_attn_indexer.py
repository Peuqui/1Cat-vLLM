...
def _torch_paged_mqa_logits(
    q_values, kv_cache, weights, seq_lens, block_tables, max_model_len, head_dim
):
    b, nn, h, d = q_values.shape
    dev = q_values.device
    logits = torch.full(
        (b * nn, max_model_len), float("-inf"), dtype=torch.float32, device=dev
    )
    bs = kv_cache.shape[1]
    ...
    for i in range(b):
        ...
        logits[i * nn : (i + 1) * nn, :length] = torch.einsum(
            "nhl,nh->nl", sc, w16
        ).float()
    return logits
